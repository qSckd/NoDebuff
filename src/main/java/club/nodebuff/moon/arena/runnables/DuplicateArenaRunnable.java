...
public abstract class DuplicateArenaRunnable extends BukkitRunnable {
    ...
    public void run() {
        if (this.paste == null) {
            Map<Location, Block> copy = this.blocksFromTwoPoints(this.copiedArena.getLocation1(),
                    this.copiedArena.getLocation2());
        ...
        new WorldEditRunnable(this.copiedArena.getSpawnA().getWorld(), this.paste) {
            @Override
            public void finish() {
                onComplete();
            }
