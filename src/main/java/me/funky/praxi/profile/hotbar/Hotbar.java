	...
	public static void init() {
		...
		for (HotbarItem hotbarItem : HotbarItem.values()) {
			try {
				...
				System.out.println("Please check your hotbar.yml config.");
			}
		}
		...
		switch (profile.getState()) {
			case LOBBY: {
				if (profile.getParty() == null) {
                    ...
                    itemStacks[6] = items.get(HotbarItem.SPECTATE_MENU);
					itemStacks[8] = items.get(HotbarItem.PROFILE_SETTINGS);
					...
					if (profile.getParty().getLeader().getUniqueId().equals(profile.getUuid())) {
						...
						itemStacks[6] = items.get(HotbarItem.PARTY_MANAGE);
						itemStacks[8] = items.get(HotbarItem.PARTY_DISBAND);
					} else {
						itemStacks[0] = items.get(HotbarItem.PARTY_INFORMATION);
						itemStacks[3] = items.get(HotbarItem.OTHER_PARTIES);
						itemStacks[8] = items.get(HotbarItem.PARTY_LEAVE);
					}
				}

				itemStacks[7] = items.get(HotbarItem.KIT_EDITOR);
			}
			break;
			case QUEUEING: {
			...
			case SPECTATING: {
                ...
                itemStacks[3] = items.get(HotbarItem.VIEW_INVENTORY);
			}
			break;
			case EVENT: {
